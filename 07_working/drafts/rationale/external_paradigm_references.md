...
The eleven sources below were read in full at the linked versions, including
...
| [Wang et al., The Long-Horizon Task Mirage?](https://arxiv.org/abs/2604.11978v1), 47 pages; empirical systems preprint | Across 3,132 trajectories in four benchmark domains, success changed nonlinearly as the authors increased task composition; their attributed failures concentrated in planning, instruction/environment handling, and memory-related behavior, with the mix depending on domain and model. | The study uses two model families, a seven-category taxonomy and an LLM judge validated on only 40 traces. The preprint contains inconsistent model labels and percentages, so its taxonomy and exact aggregate claims require replication. | Adapt with attribution: vary task-local dependency depth and inspect constraint retention and recovery, without adopting a universal horizon threshold or failure enum. |
| [Rahman et al., Locating Hidden Failures Makes Long-Horizon Agents More Reliable](https://arxiv.org/abs/2609.17930v1), 37 pages; empirical systems preprint | In 2,518 coding, terminal, and science trajectories, human first-mistake labels showed that errors often cascade and final locally correct-looking steps can hide failed runs; general-purpose judges localized first mistakes poorly, while a trained 4B verifier improved best-of-N task selection in the reported evaluations. | First-mistake localization simplifies diffuse and multi-causal failures; much of the fine taxonomy is LLM-assigned; science runs are fewer and shorter. This newly released draft contains placeholders and numerical inconsistencies, and improved benchmark selection does not establish runtime safety or human-oversight benefit. | Adapt with attribution: test independent, task-local checks of claimed postconditions and unresolved evidence; do not adopt the 88-category codebook or treat a verifier score as authority. |
...
| Can evidence-bearing checkpoints expose a long task's first consequential drift before the final answer? At a task-dependent boundary, restate the current postcondition, supporting observation, unresolved evidence, and reversible recovery point; use a separate check when consequence warrants it. | Adapts HORIZON's horizon-conditioned diagnosis and Traverse's first-mistake localization without importing either fixed taxonomy. The checkpoint content and timing come from the actual task, while a verifier remains evidence rather than authority. | Compare final-only checking, self-checking, and independent checkpoint review at matched budgets. Measure missed failures, false alarms, recovery cost, unnecessary interruptions, and unsafe actions; no improvement or higher correction burden weighs against the hypothesis. |
