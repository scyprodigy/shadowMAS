...
| `python3 05_scripts/validate/shadowmas_validate.py <packet-file>` | One YAML packet artifact (`task_packet` / `memory_packet` / `review_packet`) | Family-required field shapes, `schema_version: v0`, filename–schema major alignment, family status enum, timestamps, multi-reviewer dependency, and `source_refs` / `artifact_refs` / `handoff` shape | `0` valid · `1` validation errors · `2` usage / input / parse failure |
| `python3 tools/check_memory_validity.py` | Memory packets under `07_working/` and `examples/packets/` (or `--root`) | Ghost-dependency advisory: `broken_reference` (cited path unresolvable), `stale` (recorded source hash drifted, or declared `validity.runtime_generation` differs from `--generation`); never mutates packet status | `0` no findings · `1` findings (advisory) · `2` setup error |
...
| `python3 tools/check_packet_refs.py` | All packets under `07_working/` and `examples/packets/` | Reference-integrity gate: dangling or out-of-repo paths, duplicate packet UIDs, and unresolved `source_id`, `related_packets`, `must_read`, or promotion-snapshot edges; malformed scans fail closed | `0` no findings · `1` findings (advisory) · `2` setup / scan error |
...
| `python3 tools/check_import_conflicts.py <source> <destination> [--json]` | Two existing, separate directory trees on supported Linux hosts | Read-only portable-path and byte-identity preflight; preserves original names, refuses unsupported objects and incomplete inspection, and never copies, renames, deletes, or follows symlinks | `0` complete with no blocking findings · `1` conflicts or portability findings · `2` incomplete inspection, unsupported input, or setup error |
