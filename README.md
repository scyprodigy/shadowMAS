...
| `python3 tools/check_memory_validity.py` | Memory packets under `07_working/` and `examples/packets/` (or `--root`) | Ghost-dependency advisory: `broken_reference` (cited path unresolvable) and `stale` (recorded source hash drifted); never mutates packet status | `0` no findings · `1` findings (advisory) · `2` setup error |
